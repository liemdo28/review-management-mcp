...
class ReviewServer {
    ...
    tracker;
    constructor() {
        this.server = new Server({ name: "review-management-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
        ...
        return [
            {
                ...
                description: "Fetch reviews that have not been responded to yet across all configured " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        since_date: {
                            type: "string",
                            description: "Only return reviews on or after this date (YYYY-MM-DD). " +
            ...
            },
            {
                ...
                description: "Post a response to a review. For 1-3 star reviews you MUST set " +
            ...
            },
            {
                ...
                description: "Save a 1-3★ review to the pending queue for manager email notification " +
    ...
    async listNewReviews(args) {
        const targets = this.resolveLocations(args.location);
        const sinceDate = args.since_date ?? null;
        const lines = [];
        let totalNew = 0;
        for (const loc of targets) {
            const reviews = await this.google.getReviews(GOOGLE_ACCOUNT_ID, loc.locationId);
            const fresh = reviews.filter(r => !this.tracker.hasResponded(r.id) && // not already tracked locally
    ...
    async listAllReviews(args) {
        const targets = this.resolveLocations(args.location);
        const minStars = args.min_stars ?? 1;
        const maxStars = args.max_stars ?? 5;
        ...
        for (const loc of targets) {
            ...
            for (const r of filtered) {
                ...
                lines.push(`**Review ID**: ${r.id} [${replied}]\n` +
    ...
    async postResponse(args) {
        ...
        return {
            content: [{
                    type: "text",
                    text: `✅ Response posted to **${loc.name}**\n\n` +
    ...
    async savePendingReview(args) {
        ...
        let store = { pending: [], last_updated: "" };
        ...
        if (store.pending.find((r) => r.review_id === review_id)) {
    ...
    async getHistory(args) {
        ...
        const lines = history.map(h => `**${h.restaurant}** | ${h.timestamp.slice(0, 10)} | ` +
    ...
    // ── Utilities ─────────────────────────────────────────────────────────────
    resolveLocations(key) {
        if (!key)
            return LOCATIONS;
        const loc = LOCATIONS.find(l => l.key === key);
        if (!loc)
            throw new Error(`Unknown location key: ${key}`);
        return [loc];
    }
    // ── Start ─────────────────────────────────────────────────────────────────
    async run() {
